# -*- coding: utf-8 -*-
"""GANs_base_celebrity.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1RzP5Z3vLKGG39orsaBVOEbqhXhuzFjFk
"""
"""
#get the api from kaggle(json file) and import the dataset into colab using the following commands
...
!kaggle datasets list -s celebrity
...
transform = transforms.Compose([transforms.Scale(imageSize), transforms.ToTensor(), transforms.Normalize([0.5], [0.5]),]) # We create a list of transformations (scaling, tensor conversion, normalization) to apply to the input images.

# Loading the dataset
dataset = dset.ImageFolder(root = '/content/100k/', transform = transform)
...
x = 0
for epoch in range(15): # We iterate over 25 epochs.
    ...
    for i, data in enumerate(dataloader, 0): # We iterate over the images of the dataset.
        ...
        print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f' % (epoch, 15, i, len(dataloader), errD.data, errG.data)) # We print les losses of the discriminator (Loss_D) and the generator (Loss_G).
        if i % 100 == 0: # Every 100 steps:
            x += 1
            vutils.save_image(real, '/content/drive/My Drive/GANs Results/celebrity_results/real_sample.png' , normalize = True) # We save the real images of the minibatch.
            fake = netG(noise) # We get our fake generated images.
            vutils.save_image(fake.data, '/content/drive/My Drive/GANs Results/celebrity_results/fake_samples_epoch_%03d.png' % (x), normalize = True) # We also save the fake generated images of the minibatch.
